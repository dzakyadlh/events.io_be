...
exports.findEventsByHost = async (req, res) => {
  try {
    const { host_id } = req.params;
    const event = await EventModel.find({ host: host_id });

    if (!event) {
      return res.status(404).json({ message: 'Events not found', data: [] });
    }

    res
      .status(200)
      .json({ message: 'Events data fetched successfully', data: event });
  } catch (error) {
    res.status(500).json({ message: 'Error fetching events', error });
  }
};
